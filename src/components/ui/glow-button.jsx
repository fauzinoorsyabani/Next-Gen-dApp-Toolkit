...
/** Variants warna untuk light/dark */
const variants = {
  blue: {
    light: {
...
/** Deteksi dark mode: class .dark atau prefers-color-scheme */
const useTheme = () => {
  const [isDark, setIsDark] = useState(true); // default true krn situsmu dark-first
  useEffect(() => {
    const check = () => {
      ...
      setIsDark(darkByClass || darkByMedia);
    };
    check();
...
export default function GlowButton({ children, variant = "blue", className = "", ...props }) {
  const isDark = useTheme();
  const set = variants[variant] || variants.blue;
  const colors = isDark ? set.dark : set.light;

  return (
    <button
      {...props}
      className={`relative cursor-pointer rounded-2xl border-none p-0.5 transition-transform duration-300 ease-in-out hover:scale-105 active:scale-95 ${className}`}
    ...
    >
      ...
      >
        ...
        />
        <span className="relative z-10 whitespace-nowrap text-lg font-semibold">{children}</span>
      </div>
    </button>
  );
}
